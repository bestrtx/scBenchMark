...
#统计运行时间、资源占用
system("bash ./sc_benchmark/scripts/start_monitor.sh -n ./sc_benchmark/output/step1/LIGER/usage -t 10")
start_time <- proc.time()

scRNA <- NormalizeData(scRNA)
scRNA <- FindVariableFeatures(scRNA)
scRNA <- ScaleData(scRNA)
scRNA <- RunOptimizeALS(scRNA, k = 20, lambda = 5)
scRNA <- RunQuantileNorm(scRNA)
scRNA <- RunPCA(scRNA)
scRNA <- FindNeighbors(scRNA, reduction = "iNMF", dims = 1:20)
scRNA <- RunUMAP(scRNA, dims = 1:ncol(scRNA[["iNMF"]]), reduction = "iNMF")
...
png("./sc_benchmark/output/step1/LIGER/LIGER_umap.png", height=768, width=768)
DimPlot(scRNA, reduction = "umap", group.by = "RNA_snn_res.0.25") + ggtitle("louvain_0.25")
dev.off()
...
write.table("-----LIGER_nomorlization-----",output_txt, row.names=F, quote=FALSE, col.names = FALSE, append = TRUE)
