...
#统计运行时间、资源占用
system("bash ./sc_benchmark/scripts/start_monitor.sh -n ./sc_benchmark/output/step1/sctransform/usage -t 5")
start_time <- proc.time()

scRNA <- SCTransform(scRNA, method = "glmGamPoi", vars.to.regress = "percent.mt", verbose = FALSE)
...
for (res in c(0.05, 0.085, 0.1, 0.15, 0.25, 0.5)) {
  scRNA <- FindClusters(scRNA, graph.name = "SCT_snn", resolution = res, algorithm = 1)
}
...
plot_grid(ncol = 3, DimPlot(scRNA, reduction = "umap", group.by = "SCT_snn_res.0.15") + ggtitle("louvain_0.15"), 
          DimPlot(scRNA, reduction = "umap", group.by = "SCT_snn_res.0.25") + ggtitle("louvain_0.25"), 
          DimPlot(scRNA, reduction = "umap", group.by = "SCT_snn_res.0.5") + ggtitle("louvain_0.5"))

png("./sc_benchmark/output/step1/sctransform/sctransform_umap.png", height=768, width=768)
DimPlot(scRNA, reduction = "umap", group.by = "SCT_snn_res.0.25") + ggtitle("louvain_0.25")
dev.off()

c = paste(scRNA@assays[["SCT"]]@counts@Dimnames[[2]], scRNA@meta.data[["SCT_snn_res.0.25"]], sep="  ")
c = as.matrix(c)
...
output_txt="./sc_benchmark/output/step1/nomor_output.txt"
write.table("-----sctransform_nomorlization-----",output_txt, row.names=F, quote=FALSE, col.names = FALSE, append = TRUE)
