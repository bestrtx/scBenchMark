...
#统计运行时间、资源占用
system("bash ./sc_benchmark/scripts/start_monitor.sh -n ./sc_benchmark/output/step1/scran/usage -t 5")
start_time <- proc.time()

counts = as.matrix(dat)
sce <- SingleCellExperiment(assays = list(counts = counts))
sce <- sce[rowSums(counts(sce) > 0) > 2,]
...
png("./sc_benchmark/output/step1/scran/scran_umap.png", height=768, width=768)
DimPlot(scRNA, reduction = "umap", group.by = "RNA_snn_res.0.25") + ggtitle("louvain_0.25")
dev.off()
...
write.table("-----scran_nomorlization-----",output_txt, row.names=F, quote=FALSE, col.names = FALSE, append = TRUE)
