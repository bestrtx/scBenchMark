...
#统计运行时间、资源占用
system("bash ./sc_benchmark/scripts/start_monitor.sh -n ./sc_benchmark/output/step1/SCnorm/usage -t 5")
start_time <- proc.time()

dat = as.matrix(dat)
Conditions = rep(c(1), each= ncol(dat))

#pdf("SCnorm_check_count-depth_evaluation.pdf", height=5, width=7)
#countDeptEst <- plotCountDepth(Data = dat, Conditions = Conditions, FilterCellProportion = .1, NCores=3)
#dev.off()


#pdf("SCnorm_MyNormalizedData_k_evaluation.pdf")
#par(mfrow=c(2,2))
DataNorm <- SCnorm(Data = dat,
                   Conditions = Conditions,
                   PrintProgressPlots = TRUE,
                   FilterCellNum = 10, K = NULL,
                   NCores=10, reportSF = TRUE)
#dev.off()
...
save(NormalizedData, file = "./sc_benchmark/output/step1/SCnorm/SCnorm_NormalizedData.RData")
load("./sc_benchmark/output/step1/SCnorm/SCnorm_NormalizedData.RData")
...
png("./sc_benchmark/output/step1/SCnorm/SCnorm_umap.png", height=768, width=768)
DimPlot(scRNA, reduction = "umap", group.by = "RNA_snn_res.0.25") + ggtitle("louvain_0.25")
dev.off()
...
write.table("-----SCnorm_nomorlization-----",output_txt, row.names=F, quote=FALSE, col.names = FALSE, append = TRUE)
