...
#统计运行时间、资源占用
system("bash ./sc_benchmark/scripts/start_monitor.sh -n ./sc_benchmark/output/step2/SAVER/usage -t 60")
start_time <- proc.time()

cortex <- dat
cortex=as.matrix(cortex)
cortex.saver <- saver(cortex, ncores = 20)

dat = cortex.saver$estimate
save(dat, file = "./sc_benchmark/output/step2/SAVER/saver_data.RData")
load("./sc_benchmark/output/step2/SAVER/saver_data.RData")
...
png("./sc_benchmark/output/step2/SAVER/SAVER_umap.png", height=768, width=768)
DimPlot(scRNA, reduction = "umap", group.by = "RNA_snn_res.0.25") + ggtitle("louvain_0.25")
dev.off()
...
write.table("-----SAVER_imputation-----",output_txt, row.names=F, quote=FALSE, col.names = FALSE, append = TRUE)
