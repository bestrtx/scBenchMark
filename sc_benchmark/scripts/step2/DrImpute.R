...
#统计运行时间、资源占用
system("bash ./sc_benchmark/scripts/start_monitor.sh -n ./sc_benchmark/output/step2/DrImpute/usage -t 30")
start_time <- proc.time()

X <- preprocess(dat, min.expressed.gene = 0)
X.log <- log(X + 1)
set.seed(1)
system.time(X.imp <- DrImpute(X.log, mc.cores = 10, batch.size = 500))
colnames(X.imp) <- colnames(X.log)

end_time <- proc.time()
execution_time <- end_time - start_time
system("bash ./sc_benchmark/scripts/kill_user_usage.sh")

save(X.imp, file = "./sc_benchmark/output/step2/DrImpute/DrImpute_data.RData")
load("./sc_benchmark/output/step2/DrImpute/DrImpute_data.RData")
...
scRNA <- CreateSeuratObject(counts = X.imp, project = "Grubman", min.cells = 1, min.features = 1); scRNA
...
png("./sc_benchmark/output/step2/DrImpute/DrImpute_umap.png", height=768, width=768)
DimPlot(scRNA, reduction = "umap", group.by = "RNA_snn_res.0.25") + ggtitle("louvain_0.25")
dev.off()
...
write.table("-----DrImpute_imputation-----",output_txt, row.names=F, quote=FALSE, col.names = FALSE, append = TRUE)
