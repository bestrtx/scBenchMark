...
#读入
dat <- read.table("./sc_benchmark/output/step2/deepImpute/imputed.csv",row.names = 1,header = T,sep=",")
dat=t(dat)
...
png("./sc_benchmark/output/step2/deepImpute/DeepImpute_umap.png", height=768, width=768)
DimPlot(scRNA, reduction = "umap", group.by = "RNA_snn_res.0.25") + ggtitle("louvain_0.25")
dev.off()
...
write.table("-----deepImpute_imputation-----",output_txt, row.names=F, quote=FALSE, col.names = FALSE, append = TRUE)
